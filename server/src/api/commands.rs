...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Commands registered", body = Vec<CommandResponse>),
        (status = 400, description = "Invalid command data"),
        (status = 404, description = "Application not found"),
        (status = 403, description = "Forbidden"),
        (status = 409, description = "Duplicate command name"),
    ),
    security(("BearerAuth" = []))
)]
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 404, description = "Command or application not found"),
        (status = 403, description = "Forbidden"),
    ),
    security(("BearerAuth" = []))
)]
