...
const CreateChannelModal: Component<CreateChannelModalProps> = (props) => {
  ...
  const [channelType, setChannelType] = createSignal<"text" | "voice">(props.initialType || "text");
  ...
  return (
    <Portal>
      <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
        <div class="border border-white/10 rounded-2xl w-[440px] flex flex-col shadow-2xl" style="background-color: var(--color-surface-base)">
          {/* Header */}
              ...
              <div>
                ...
                <div class="flex gap-3">
                  <button
                    ...
                    classList={{
                      "border-accent-primary bg-accent-primary/10": channelType() === "text",
                      "border-white/10 hover:border-white/20": channelType() !== "text",
                    }}
                  >
                    <div class="w-10 h-10 rounded-lg flex items-center justify-center" style="background-color: var(--color-surface-layer2)">
                      <Hash size={20} class="text-text-primary" />
                    </div>
                    <div class="text-left">
                      <div class="font-semibold text-text-primary">Text</div>
                      <div class="text-xs text-text-secondary">Send messages</div>
                    </div>
                  </button>
                  <button
                    type="button"
                    ...
                    class="flex-1 flex items-center gap-3 p-4 rounded-xl border-2 transition-all"
                    classList={{
                      "border-accent-primary bg-accent-primary/10": channelType() === "voice",
                      "border-white/10 hover:border-white/20": channelType() !== "voice",
                    }}
                  >
                    <div class="w-10 h-10 rounded-lg flex items-center justify-center" style="background-color: var(--color-surface-layer2)">
                    ...
                    <div class="text-left">
                      ...
                      <div class="text-xs text-text-secondary">Talk with others</div>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="relative">
                  <div class="absolute left-4 top-1/2 -translate-y-1/2 text-text-secondary">
                    {channelType() === "text" ? <Hash size={18} /> : <Mic size={18} />}
                  </div>
                  <input
                    type="text"
                    value={name()}
                    onInput={(e) => setName(e.currentTarget.value.toLowerCase().replace(/\s+/g, "-"))}
                    placeholder={channelType() === "text" ? "general-chat" : "voice-lounge"}
