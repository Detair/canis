...
- **Always enforce timeouts** — To prevent defunct processes from hanging in the background, wrap potentially blocking scripts or build commands with the `timeout` utility (e.g., `timeout 30s bun run lint` or `timeout 60s python3 script.py`).
